...
const Page = () => {
  ...
  console.log("我被处罚了",pathname);
  const configMap = useRef<Record<string,any>>()
  const { usersStore } = useStores();
  const { drawerFormData } = usersStore;
  const { pending } = useFormStatus();
  const formData = useMemo<any>(() => JSON.parse(JSON.stringify(drawerFormData)), [drawerFormData])
  ...
  if (formData && formData?.columns && Array.isArray(formData.columns)) {
    const config = formData.columns
      .filter(({ formType }: { formType: string | undefined }) => !!formType && formType === 'input')
      .map((items: any) => {
  ...
  return <DrawerContent forceMount>{JSON.stringify(drawerFormData)}

    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
        <FormFields config={config} form={form} />
        <Button type="submit" disabled={pending}>测试</Button>
      </form>
    </Form>
  </DrawerContent>;
};
